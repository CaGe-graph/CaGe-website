#define VERSION "3.1 - July 5 2000"
#define SWITCHES "[-uagsh -c#txm#P#bpe#f# -odG -v]"

/* plantri.c :  generate planar triangulations    Version 3.1

  For more detailed information, see the separate manual plantri-guide.txt.

Types of graphs produced:

  In all cases, graphs are imbedded in the plane and isomorphisms must
  preserve the imbedding.  Such isomorphisms can be either orientation
  preserving or orientation independent (allowing mirror image).  Normally
  orientation independent isomorphisms are meant, but there is an option
  (-o) for selecting only orientation preserving isomorphisms.

  The word "triangulation" means that each face is bounded by three edges,
  including the outside face.  There is one standard exception to this
  terminology: in the case of "triangulations of a polygon", also called
  "triangulations of a disk", the outside face can have any size.

  "k-conn" means "vertex connectivity at least k"
  "mindeg" means "minimum vertex degree", aka "minimum valence"

  Now we list the major combinations of options and what they do.  In all
  case the default output is the "primal form", but the "dual form" can
  be requested by adding the -d option.  All graphs are simple (no loops
  or parallel edges) unless explicitly stated otherwise.

  Option            Primal form                     Dual form

    -p        3-conn planar graph           3-conn planar graph

       These are also known as planar polytopes or 3-dimensional polytopes.  
       Bounds on the number of edges, or an upper bound on the face size,
       can be specified using -e or -f.  You can also specify a mindeg of
       at least 4 by using -m4.

    -P      triangulation of disk            dual of that

       The argument to P gives the size of the outside face (3..n).
       If there is no argument, all possible sizes are produced.  Normally
       only 3-connected triangulations are made.  If -c2 is given, it is
       permissible for the outside face to have chords, but there are no
       other 2-cuts and the mindeg is at least 3.  If -c2m2 is given, the
       outside face can have vertices of degree 2 (but there are still no
       2-cuts except for chords of the outside face).

       The dual is a cubic graph except that one vertex can have higher
       degree.  In the case -c2m2 (only) that vertex may have double edges.

   For further options we assume neither -p nor -P are present.

    -m4  3-conn planar triangulation       3-conn planar cubic graph
         with mindeg at least 4            without triangles

    -c4  4-conn planar triangulation       planar cubic graph with cyclic
                                           connectivity at least 4
 
    -b   3-conn planar triangulation       3-conn planar cubic bipartite graph
         with each vertex of even degree

   -bc4  4-conn planar triangulation       planar cubic bipartite graph
         with each vertex of even degree   with cyclic connectivity >= 4

   -c2   2-conn planar triangulation       2-conn planar cubic graph
         with minimum degree >= 3

   -c1   1-conn planar triangulation with    1-conn planar cubic graph
         mindeg >= 3 and no two faces
         sharing more than one edge

  -c1t   1-conn planar triangulation       1-conn planar cubic graph
         with mindeg >= 3                  with face sizes >= 3

  default  3-conn planar triangulation     3-conn planar cubic graph

       The primal forms (triangulations) are simple if neither -c1 or -c2
       are given.  With -c2 parallel edges may exist, and with -c1 loops
       may exist as well.  The dual forms (cubic graphs) are all simple
       except in the case -c1t, when parallel edges may exist.


  Usage:  plantri [-aghoduxc#GtP#bpe#f#] n [res/mod] [outfile]

      n is the number of vertices in the triangulation (4-MAXN)
           You can also specify this as the number of faces by following
           the number with a 'd'.  For example, 10 is the same as 16d.
           The number of faces equals the number of vertices in the planar
           cubic graph which is the dual of the triangulation.
           For polytopes, -p option, a 'd' refers to the maximum face
           count (as for a triangulation) not to a face count constant
           between outputs.
           For disk triangulations, -P option, a 'd' refers to the number
           of faces that would result if the outside of the polygon
           was triangulated with chords.

      outfile is the output file  (missing or "-" means stdout)

      res/mod are two integers for selecting just some part of the
           output.  It must be that 0 <= res < mod.
           For example 3/7 selects subset 3 out of 0,1,...,6.

      -o : Without this option, mirror images are treated as the same.  
           With this option they are different unless there is a 
           reflecting automorphism.  Implies -G.  In the case of -g or -s
           only one graph is written because graph6 and sparse6 formats
           lose the imbedding.  Therefore it is usually a waste of time
           to use -o and (-g or -s) together.

      -u : Just count, don't write any graphs.

      -d : Write the dual graph instead (a cubic planar graph).

      -a : Write in a human-readable ascii text format, instead
           of the default planar_code format
           
      -g : Write in graph6 format, instead of the default planar_code format.
           -a, -s and -g are incompatible.  Note that loops are lost and
           multiple edges are collapsed into single edges, as graph6
           format only supports simple graphs.

      -s : Write in sparse6 format, instead of the default planar_code format.
           -a, -g and -s are incompatible
           sparse6 is shorter for cubic graphs of 20 or more vertices.

      -h : Without -a or -g, suppress writing of header ">>planar_code<<".
           With -g, force writing of header ">>graph6<<".
     ...
     -c# : Specify minimum connectivity of the dual cubic graph, same
           as the shortest cycle length in the triangulation.  Legal
           values are 1,2,3.  In the case of triangulations of a
           polygon (-P option) legal values are 2 and 3. Default is
           3 in both cases.

     -m# : Specify a lower bound on the minimum degree (minimum face
           size for the dual).  The only cases currently available are
           -m4 for ordinary 3-connected triangulations, and -m2 for
           polygon triangulations.  -m3 is also legal (and default) for
           both those cases.

      -t : Produce all connected triangulations with no vertices of
           degree less than 3.  This only makes a difference if -c1
           is also given.

      -P :
     -P# : Produce triangulations of a polygon, rather than triangulations
           of a sphere.  The value is the size of the polygon, unrestricted
           if there is no value.  Incompatible with -c1, -t, -p, -b.  The n
           parameter is the total number of vertices as always.  Can be
           used with -c2 and -c3 (default), and with -x.  The only 2-cuts
           allowed with -c2 are chords of the polygon.   -P without a
           value and -P0 are the same.

           The outside face can be found to the left of the edge 0-1
           in the primal output.  For the dual output, vertex 1 is
           the outer face.

      -x : Only output those with exactly the connectivity specified
           by the -c switch.
           
      -p : Produce polytopes.  These are 3-connected planar graphs.
           Incompatible with -P, -c1, -c2, -t and -b.

     -e# -e:# -e#: -e#:# :
           Bounds on the number of edges for polytopes.  Illegal without -p.
           The four forms mean:  exactly that number of edges, at most
           that number, at least that number, between these limits inclusive.
           Note that only the lower bound is implemented efficiently!

     -f# : The maximum face size for polytopes.  Illegal without -p.

      -v : Provide additional statistical information to stderr.
...
---------
   ...
   Output formats: 
      ...
      encode the imbedding.  They are described in plantri-guide.txt.

---------

   Counts:

     vertices   edges  faces    without -o      with -o
       ...
       21        55     38  453382272049
       
Estimates (very accurate):
 
       22        60     40    3.58585e12     double
       23        63     42    2.86157e13    previous
       24        66     44    2.30215e14     column
       25        69     46    1.86579e15       "
...
bits in a long (usually 32 or 64).
...
Change History:
       ...
       20-Sep-1998 : improved performance about 25%
        ...
        2-Jul-2000 : fixed an error causing problems for minimum
                     ...
                     to Thom Sulanke for find it.  (Two changes in
                     scansimple().
                     -- making 3.1
...
#ifndef MAXN
#define MAXN 100            /* the maximum number of vertices; see above */
#endif
...
static int minconnec;      /* minimum connectivity */
...
static long am[MAXN];
  /* adjacency matrix, only used for maxconnec < 3. */
#define BIT(i) (1L << (i))
...
static bigint totalout;    /* Sum of nout[] */
static bigint noutisom;   /* With -o, count isomorphism classes */
...
/* 
   ...
   It is only compared with the other edges contained in edgelist and their 
   inverse. The number of those edges in the list is given in num_edges.
   ...
   and if doflip != 0 also write in prev direction. */
{
    ...
    length=nv+ne+1;
    compute_code(code);
    if (fwrite(code,sizeof(unsigned char),(size_t)length,f) != length)
    {
...
/* Write a graph represented in planar_code to a file in
   sparse6 format.  The imbedding is lost but the vertex
   numbering is the same.  This goes not use any global
   variables and works to 255 vertices. */
{
...
/* Write a graph represented in planar_code to a file in
   graph6 format.  The imbedding is lost but the vertex
   numbering is the same.  This goes not use any global
   variables and works to 255 vertices. */
{
...
/* Checks these properties:
   1. Degrees are >= 3 and correct.
   2. Faces are triangles. 
   ...
   purpose is to write the triangulation and to collect some stats. */
{
    ...
    if (xswitch && connec != minconnec) return;
    ...
    if (vswitch && oswitch) ADDBIG(noutisom,1);
    ADDBIG(nout[connec],doflip+1);
    if (pswitch)
    {
   ...
   obtained by deleting e). */
{
    ...
    EDGE *run, *start, *end, *endrun;

    RESETMARKS_V;

    MARK_V(e->end); MARK_V(e->start);
...
/* The first face shares a whole edge with  the new face
       -- this is legal, so only the second vertex must be considered */
    run=start->invers->prev;
    start=start->next;
    endrun=start->invers;
    for ( ; run != endrun; run=run->invers->prev)
        if (ISMARKED_V(run->end)) return(0); 

/* for the next faces, already the first entry can give a contradiction */
...
/* for the last face the last edge is shared with the new face, so this
   must not be considered */

    endrun=start->next->invers->next->invers;
    for (run=start ; run != endrun; run=run->invers->prev)
	if (ISMARKED_V(run->end)) return(0); 
...
static void
make_3conn_list(EDGE *old[], int numold, EDGE *new[], int *numnew)
...
/* Reduce edge[0..*nedges-1] according to the group */
{
    int i,oldnum,newnum;
    EDGE **nb0,**nb,**nblim;
...
*/
{
#define DEGEND(ed) (degree[ed->end])
#define REJECT {++degree[v1]; ++degree[v2]; return 2;}
#define ORTEST(e) {col = DEGEND(e); \
 if (col > maxcol) REJECT else if (col == maxcol) good_or[ng_or++] = e;}
#define INVTEST(e) {col = DEGEND(e); \
 if (col > maxcol) REJECT else if (col == maxcol) good_inv[ng_inv++] = e;}
#define ORCOL(ed) ((degree[ed->invers->prev->end] << 10) + ed->left_facesize)
#define INVCOL(ed) \
 ...
 /* Now old edges that will be on a largest face (if *edel is deleted)
    ...
    if (maxface > 4)
    {
	if (degree[v2] == maxdeg) ORTEST(e3);
	if (degree[v1] == maxdeg) INVTEST(e4);
    }
   ...
   left face has greatest size (unless that is 3). */
{
	...
	if (e->invers->left_facesize < oldmaxface-1
	    ...
	    if (ngood_or + ngood_inv == 1)
	    {
                ...
                make_3conn_list(oldfeas,noldfeas-1,newfeas,&nnewfeas);
                oldfeas[i] = esave;
...
/* This routine begins the scan for  3-conn planar graphs formed by
   removing edges from 3-conn triangulations.  The current graph is such
   a triangulation, and the group is known (parameters nbtot,nbop). */
{
    ...
    for (i = 0; i < nv; ++i)
    {
        ...
        do
        {
            if (e == e->min && degree[e->start] > minconnec
                            && degree[e->end] > minconnec)
                feasible[nfeas++] = e;
	    e->left_facesize = 3;
            e = e->next;
        } while (e != ex);
    ...
    make_3conn_list(feasible,nfeas,feasible,&nfeas);

    scanpoly(nbtot,nbop,feasible,nfeas,3,0,0);
}
        ...
        if (degree5_vertex() < 0 && min4_four_centre() < 0)
        {
            ...
            if (ngood_ref+ngood_mir_ref > 0)
            {
                                        ...
                                        zero,numbering,&xnbtot,&xnbop))
                {
                    ...
                    e = e->invers->prev->prev->prev;
                    nft[numnft].e3 = e->min;
                    scanmin4c(xnbtot,xnbop,nv>=splitlevel&&nv<=splitlevel+2,
                                            NULL,nft,numnft+1);
                }
            }
        }
        ...
        if (degree5_vertex() < 0 && min4_four_centre() < 0)
        {
            ...
            if (ngood_ref+ngood_mir_ref > 0)
            {
                if (canon_edge_oriented(good_or,ngood_or,ngood_ref,
                                        good_mir,ngood_mir,ngood_mir_ref,
                                        zero,numbering,&xnbtot,&xnbop))
                scanmin4(xnbtot,xnbop,nv>=splitlevel&&nv<=splitlevel+2,NULL);
            }
        }
        reduce_S(extS[i]);
    }
...
*/
{
    ...
    for (i = 1; !badargs && i < argc; ++i)
    {
        ...
        for (j = 1; arg[j] != '\0'; ++j)
            ...
            else if (arg[j] == 'v') vswitch = TRUE;
            else if (arg[j] == 'x') xswitch = TRUE;
            else if (arg[j] == 't') tswitch = TRUE;
            else if (arg[j] == 'p') pswitch = TRUE;
            else if (arg[j] == 'b') bswitch = TRUE;
            else if (arg[j] == '0') zeroswitch = TRUE;
            else if (arg[j] == 'e')
	    {
		...
		if (edgebound[1] == 0) edgebound[1] = MAXE/2;
            ...
            else if (arg[j] == 'f') maxfacesize = getswitchvalue(arg,&j);
            else if (arg[j] == 'c') minconnec = getswitchvalue(arg,&j);
            else if (arg[j] == 'P') polygonsize = getswitchvalue(arg,&j);
            else if (arg[j] == 'm') minimumdeg = getswitchvalue(arg,&j);
...
/* Polytopes.  Main options are -m4 and not -m4. */
{
    int startingsize,nbtot,nbop,i,hint;

    if (minconnec <= 0) minconnec = 3;
    if (minimumdeg <= 0) minimumdeg = minconnec;
    startingsize = minimumdeg == 4 ? 6 : 4;

    CHECKRANGE(maxnv,"n",startingsize,MAXN);
    CHECKRANGE(minconnec,"-c",3,3);
    CHECKRANGE(minimumdeg,"-m",3,4);

    INCOMPAT(polygonsize>=0,"-p","-P");
    INCOMPAT(bswitch,"-p","-b");
    INCOMPAT(tswitch,"-p","-t");
    
    if (minpolydeg < 0) minpolydeg = minimumdeg;

    if (edgebound[0] < 0) edgebound[0] = 0;
    if (edgebound[1] < 0) edgebound[1] = 3*maxnv-6;
    if (edgebound[1] > 3*maxnv-6) edgebound[1] = 3*maxnv-6;
    if (maxfacesize <= 0) maxfacesize = maxnv-1;
    ...
    CHECKRANGE(maxfacesize,"-f",3,maxnv-1);

    if (edgebound[0] == 3*maxnv-6 || maxfacesize == 3)
    {
	pswitch = FALSE;
	edgebound[0] = edgebound[1] = maxfacesize = -1;
        if (minconnec >= 4 || minimumdeg >= 4) min4_dispatch();
...
main(int argc, char *argv[])
{
    ...
    totalout = nout[0];
    SUMBIGS(totalout,nout[1]);
    SUMBIGS(totalout,nout[2]);
    SUMBIGS(totalout,nout[3]);
    SUMBIGS(totalout,nout[4]);
    SUMBIGS(totalout,nout[5]);
    ...
    if (oswitch && vswitch)
    {
	PRINTBIG(msgfile,noutisom);
	fprintf(msgfile," isomorphism classes; ");
	SUMBIGS(noutisom,noutisom);
	SUBBIGS(noutisom,totalout);
	PRINTBIG(msgfile,noutisom);
	fprintf(msgfile," reflection invariant\n");
    }
