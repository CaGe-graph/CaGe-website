...
#define S        105           /* Maximale Anzahl der 6-Ecke */
...
{
int i;
if (laenge==8) { fprintf(stderr,"%d %d    \n",code[0],code[1]);
		 i=2; }
else i=0;
...
int dualcode( FILE *fil, PLANMAP map )

{
/* berechnet das Dual und schreibt es als planarcode auf fil */
...
fwrite(code,sizeof(FLAECHENTYP),codelaenge,fil);
...
int longcode( PLANMAP map, FLAECHENTYP *codeF, KNOTENTYP *codeK )
{
...
{ int zaehler, i,j, mark, sixgons;
  ...
  FLAECHENTYP codeF[81+8*S];
  KNOTENTYP codeK[81+8*S];
  FILE *fil2;
...
if (!gefunden) { no_penta_spiral++;
		 zaehler=longcode(map,codeF,codeK);
		fil2=fopen(no_penta_spiral_filename,"a");
		if (knotenzahl <= FL_MAX) fwrite(codeF,sizeof(FLAECHENTYP),zaehler,fil2);
		  else { fputc(0,fil2); fwrite(codeK,sizeof(KNOTENTYP),zaehler,fil2); }
		fclose(fil2);
	       }
...
if (hexspi && !hexgefunden) { no_hexa_spiral++;
		 zaehler=longcode(map,codeF,codeK);
		fil2=fopen(no_hexa_spiral_filename,"a");
		if (knotenzahl <= FL_MAX) fwrite(codeF,sizeof(FLAECHENTYP),zaehler,fil2);
		  else { fputc(0,fil2); fwrite(codeK,sizeof(KNOTENTYP),zaehler,fil2); }
		fclose(fil2);
	       }
...
switch(codenumber)
  ...
  case 0: {
	     ...
	     { test=spiralcode(map,codeF); 
	       if (!test)
		 { zaehler=longcode(map,codeF,codeK);
		   fil2=fopen(no_spiral_filename,"a");
		   if (knotenzahl <= FL_MAX) fwrite(codeF,sizeof(FLAECHENTYP),zaehler,fil2);
		   else { fputc(0,fil2); fwrite(codeK,sizeof(KNOTENTYP),zaehler,fil2); }
		   fclose(fil2);
		 }
  ...
  case 1: { 
		 ...
		 { zaehler=longcode(map,codeF,codeK);
		   ...
		   fclose(fil2);
            ...
            zaehler=longcode(map,codeF,codeK);
	    if (knotenzahl <= FL_MAX) fwrite(codeF,sizeof(FLAECHENTYP),zaehler,fil[knotenzahl]);
		  else { fputc(0,fil[knotenzahl]); 
			 fwrite(codeK,sizeof(KNOTENTYP),zaehler,fil[knotenzahl]); }
	    break; }
  ...
  case 3: { 
	    ...
	    if (test) { in_liste(codeF,codeliste+knotenzahl);
			if (graphenzahl[knotenzahl]%listenlaenge==0)
			  {
			  codeFPZ = *(lastcode[knotenzahl]);
...
/* nur zum testen -- spaeter raus: Sequenzen vergleichen */

void seq_vgl(SEQUENZ s, SEQUENZ t)
{
int i;

if (s.laenge != t.laenge) { fprintf(stderr,"Sequenzen ungleich \n"); exit(20); }



if (s.laenge==0) 
 { if (s.sequenz[0] != t.sequenz[0]) { fprintf(stderr,"Sequenzen ungleich laenge 0, %d %d \n",s.sequenz[0], t.sequenz[0]); exit(21); }
if (s.kanten[0] != t.kanten[0]) { fprintf(stderr,"Sequenzen ungleich laenge 0, %d %d \n",s.kanten[0], t.kanten[0]); exit(22); }
 }
else
for (i=0; i<s.laenge; i++)
 { if (s.sequenz[i] != t.sequenz[i]) { fprintf(stderr,"Sequenzen ungleich laenge %d, stelle %d, %d %d \n",s.laenge,i,s.sequenz[i], t.sequenz[i]); exit(23); }
if (s.kanten[i] != t.kanten[i]) { fprintf(stderr,"Sequenz_kanten ungleich laenge %d stelle %d, %d %d \n",s.laenge,i, s.kanten[i], t.kanten[i]); exit(24); }
if (s.k_marks[i] != t.k_marks[i]) { fprintf(stderr,"Sequenz_kmarks ungleich laenge %d stelle %d, %d %d \n",s.laenge,i,s.k_marks[i], t.k_marks[i]); exit(25); }
 }
} 
...
void reko_bb_code(PLANMAP map, FLAECHENTYP *code, FLAECHENTYP *spiegelcode, int laenge, 
...
for ( run2=run->prev; verschiebung<=laenge_2; verschiebung++, run2=run2->prev->invers->next->invers)
  if (checksize(run2)==5)
	...
	{ 
      ...
      if ((test=codecmp(code+2,testcode+2,6))>=0) 
...
for ( run2=run->prev; verschiebung<=laenge_2; verschiebung++, run2=run2->invers->prev->invers->next)
  if (checksize(run2)==5)
	...
	{ flaechennumber++;
      ...
      if ((test=codecmp(spiegelcode+2,testcode+2,6))>=0) 
...
int bauchbinde_reko(PLANMAP map,FLAECHENTYP *code,KANTE *anfangskante)
{
int i,j,k, patchknz1, patchknz2, test, test2, test3, test4, returnwert;
KANTE *run;
FLAECHENTYP code1[8], code2[8], spiegelcode1[8], spiegelcode2[8];
KANTE *ministart1[7], *ministart2[7], *spiegelministart1[7], *spiegelministart2[7];
FLAECHENTYP *ucode, *lcode, *uscode, *lscode;
...
if (patchknz2>patchknz1) { ucode=code2; uscode=spiegelcode2; lcode=code1; lscode=spiegelcode1; 
	  ...
	  { test=codecmp(code1,code2,8);
	    ...
	    test=codecmp(spiegelcode1,spiegelcode2,8);
...
if ((test=codecmp(lcode,code+2,8)) < 0) {code[3]=c3; code[11]=c11; return(0);}
   else if (test==0)
         { if ((test2=codecmp(ucode,code+10,8)) < 0) {code[3]=c3; code[11]=c11; return(0);}
...
if ((test3=codecmp(lscode,code+2,8)) < 0) { code[3]=c3; code[11]=c11; return(0); }
   else if (test3==0)
         { if ((test4=codecmp(uscode,code+10,8)) < 0) {code[3]=c3; code[11]=c11; return(0);}
...
void reko_patch(KANTE *anfang,FLAECHENTYP code[],int fuenfecke)
{
...
if ((test=codecmp(testcode,code+vergleichsanfang,fuenfecke))<0) return(0);
if (test>0) return(1);
...
void reko_patch_sp(KANTE *anfang,FLAECHENTYP code[],int fuenfecke)
{
int i, position, zaehler, z2;
KANTE *run, *merke;
...
int brille_sp_reko(PLANMAP map,FLAECHENTYP *code,KANTE *anfangskante)
{
int i,j, l1, l2, l3, fuenfecke, vergleichsanfang, test;
KANTE *run, *startkante1, *startkante2, *startkante3, *anfang;
FLAECHENTYP testcode[6];
...
int sandwich_reko(PLANMAP map,FLAECHENTYP *code,KANTE *anfangskante)
{
...
int sandwich_reko_sp(PLANMAP map,FLAECHENTYP *code,KANTE *anfangskante)
{
int i,j, l1, l2, l1_p_l2, fuenfecke, vergleichsanfang, test;
KANTE *run, *startkante1, *startkante2, *startkante3, *anfang;
FLAECHENTYP testcode[6];
...
/* Bleibt drin, da es eventuell noch nuetzlich sein kann, wenn man z.B 
   Fullerene sucht, die nur Brillen haben, oder so */


int minitest_alt(PLANMAP map, FLAECHENTYP *code)
...
{
int i, j, k, m_laenge, ms_laenge;
KANTE *miniliste[3*N], *mini_spiegel[3*N];
KANTE *anfangskante, *run, *run2;
long mark[N+1];
int zaehler, anfang, stelle_1, stelle_2;
FLAECHENTYP cpcode[20];
int laenge_1, laenge_2, laenge_3;
int kantenzaehler, test;
...
for (i=1; i<=map[0][0].name; i++) { mark[i]=0;
				    for(j=0;j<3;j++) 
				      { run=map[i]+j;
					run->dummy=run->nostart=run->mirror_nostart=run->mininame=0;
				      }
				  }
if (code[1]!=1) map[1][0].invers->nostart=1;


for (i=1; i<=map[0][0].name; i++) 
  for (j=0; j<3; j++)
    { 
    anfangskante=map[i]+j;
    if (!anfangskante->nostart)
    {
      if (zaehler % 2) zaehler++;
        else zaehler+=2; /* immer mit einem geraden Wert anfangen */
      mark[i]=anfang=zaehler; 
      zaehler++;  /* anzahl der kanten */
      stelle_1=stelle_2=0;
      anfangskante->dummy=anfangskante->invers->dummy=anfang; /* markierung fuer "erreicht" */
      mark[anfangskante->name]=zaehler;
      run=anfangskante->invers->prev;
      kantenzaehler=1;
      while ((kantenzaehler<=code[0]) && (run->dummy != anfang))
	{ 
	  run->dummy=run->invers->dummy=anfang;
	  zaehler++; kantenzaehler++;
	  if (mark[run->name] >= anfang)
	    { if (stelle_1) stelle_2=mark[run->name]; else stelle_1=zaehler; }
	  else mark[run->name]=zaehler;
	  if (zaehler%2) run=run->invers->prev; else run=run->invers->next;
	}
      /* Jetzt auswerten */
      if (kantenzaehler<=code[0])
      {
      if (stelle_2) /* d.h. Brille oder Sandwich gefunden */
	{
	  if (stelle_2 > stelle_1) /* Brille */
	    { laenge_1=stelle_1-anfang; laenge_3=zaehler-stelle_2; 
	      if (laenge_1 >= laenge_3)
		{ cpcode[0]=kantenzaehler; cpcode[1]=2; cpcode[2]=laenge_1; cpcode[3]=laenge_3; }
	      else cpcode[0]=unbelegt;
	    }
	  else /* d.h. sandwich */
	    { laenge_1=stelle_2-anfang; laenge_2=stelle_1-stelle_2; laenge_3=zaehler-stelle_1;
	      if (laenge_1 >= laenge_3)
		{ cpcode[0]=kantenzaehler; cpcode[1]=3; cpcode[2]=laenge_1; cpcode[3]=laenge_2; }
	      else cpcode[0]=unbelegt;
	    }
	  if ((test=codecmp(cpcode,code,4)) <0) return(0);
	  if (test==0) { miniliste[m_laenge]=anfangskante; m_laenge++; }
	  } /* ende if (stelle_2) */
      else
	{ if ((run==anfangskante) && !(zaehler%2)) 
	     { cpcode[0]=kantenzaehler; cpcode[1]=1;
	     run2=anfangskante;
	       for (k=0; k<kantenzaehler; k+=2)
		 { 
		   run2->nostart=run2->invers->nostart=1;
		   run2=run2->invers->prev;
		   run2->mirror_nostart=run2->invers->mirror_nostart=1;
		   run2=run2->invers->next; }
	     }
	    else { cpcode[0]=unbelegt; }
	  if ((test=codecmp(cpcode,code,2)) <0) return(0);
	  if (test==0) { miniliste[m_laenge]=anfangskante; m_laenge++; }
	}
      }/* ende kantenzaehler <= code[0] */
    }

/* Jetzt zum Spiegelbild: */

    if (!anfangskante->mirror_nostart)
    {
      if (zaehler % 2) zaehler++; 
         else zaehler +=2; /* immer mit einem geraden Wert anfangen */
      mark[i]=anfang=zaehler; 
	...
	{ 
	  ...
	  if (zaehler%2) run=run->invers->next; else run=run->invers->prev;
	}
      /* Jetzt auswerten */
      if (kantenzaehler<=code[0])
      {
	...
	{ if (stelle_2 > stelle_1) /* Brille */
	  ...
	  if ((test=codecmp(cpcode,code,4)) <0) return(0); 
	  if (test==0) { mini_spiegel[ms_laenge]=anfangskante; ms_laenge++; }
	  } /* ende if (stelle_2) */
      /* else faellt aus, da bauchbinden immer in beide Richtungen durchlaufen werden koennen, also auch
	 schon von der Nicht-Spiegel-Richtung aus getestet wurden */
      }/* ende kantenzaehler <= code[0] */
   }  
  } /* ende for ueber i und j */



if (code[1]==1) /* Bauchbinde */
  { for (i=0; i<m_laenge; i++)
       if (bauchbinde_reko(map,code,miniliste[i])==0) return(0);
  }
else
  if (code[1]==2) /* Brille */
     { for (i=0; i<m_laenge; i++)
	 if (brille_reko(map,code,miniliste[i])==0) return(0);
       for (i=0; i<ms_laenge; i++)
	 if (brille_sp_reko(map,code,mini_spiegel[i])==0) return(0); 
     }
    else /* d.h. (code[1]==3) d.h. Sandwich */
  { for (i=0; i<m_laenge; i++)
       if (sandwich_reko(map,code,miniliste[i])==0) return(0);
    for (i=0; i<ms_laenge; i++)
       if (sandwich_reko_sp(map,code,mini_spiegel[i])==0) return(0); 
  }
...
int minitest(PLANMAP map, FLAECHENTYP *code)
...
if (code[1]==1) { rekonstruiert=1; /* Bei bauchbinden ist das immer der Fall */
  ...
  { for (i=0; (i<60) && !rekonstruiert; i++) 
			   ...
			   if (run->ursprung <= knoten_auf_pfad) 
				 ...
				 { if ((run->ursprung <=l1) || (run->ursprung >= l1+l2)) rekonstruiert=1;
			          else { if (run->prev->ursprung==l1+l2)
					   { if ((l1+l2) % 2) rekonstruiert=1; }
					 else
					   if (run->name==l1)
					     { if ((l2+l3) % 2) rekonstruiert=1; }
...
if (cpcode[1]==1) { if ((test=codecmp(code,cpcode,2))>0) return(0); }
      else { if ((test=codecmp(code,cpcode,4))>0) return(0); }
...
/* nun in die andere Richtung zuerst */
	...
	if ((test=codecmp(code,cpcode,4))>0) return(0); 
...
if (code[1]==1) {      /* Bauchbinde  (redundante if-Abfrage) */
  ...
  if (codecmp(&code[2],&code[10],8)==0) {               /* lowercode==uppercode */
    k1 = miniliste[0];               /* erste Kante */   
    j=1;  i=0;
    while (j<=knoten_auf_pfad/2) {     /* den halben Pfad entlanggehen */
      ...
      if (!run->dummy)
	{
	  ...
	  if (run->name != aussen)
	    if ((checksize_2(run)==5) && (checksize_right_2(run)==5)) 
	      { fprintf(stderr,"%d-->%d\n",run->ursprung,run->name); schreibemap(map); exit(85); }
	}
    }
return(1);
}
...
void teste_und_schreibe(PLANMAP map, FLAECHENTYP *code)
{ int erg;
  char i;
/* vorerst: schreibemap(map);*/
	       ...
	       if (!mod || ((modulozaehler % mod) == rest))
		 {
		   ...
		   for (laenge_3=1; ((randl_p1 + laenge_3) <= maxpfadlaenge) && (laenge_1>= laenge_3); laenge_3++)
			 ...
			 { code[0]=laenge_3 + randl_p1;
			   ...
			   /* fuer das patch zwischen l1 und l3 ist die Lage leider komplizierter */ 	
...
if (splitter)  { if (do_case) 
                    sprintf(befehl,"splitter n fuco_xx_%d_%d b %s",do_case,knotenzahl,argv[splitter]);
	          else sprintf(befehl,"splitter n fuco_xx_%d b %s",knotenzahl,argv[splitter]);
                 filsplit=popen(befehl,"w");
                 if (filsplit==NULL) { fprintf(stderr,"Can not open pipe for splitter. \n");
                                       exit(108); }
                }
...
switch (codenumber)
  {
  case 0: break;
  case 1: { for (i=20+(2*min_sechsecke); i<=knotenzahl; i+=2)
		...
		if (to_stdout) fil[i]=stdout; else if (splitter) fil[i]=filsplit; 
                                                     else fil[i]=fopen(strpuf,"w");
  ...
  case 2: { for (i=20+(2*min_sechsecke); i<=knotenzahl; i+=2)
		...
		if (mod) { sprintf(strpuf2,"_m_%d_%d",rest,mod); strcat(strpuf,strpuf2); }
                if (symm_len>0)    {strcat(strpuf,symmstring);}
		if (to_stdout) fil[i]=stdout; else fil[i]=fopen(strpuf,"w");
	        if (fil[i]==nil) { fprintf(stderr,"Can not open file %s. \n",strpuf); exit(110);}
	      }
	    spiralcheck=1;
	    last_code[0]=FL_MAX;
...
if (splitter && (((codenumber!=1) && (codenumber!=5)) || to_stdout || (min_sechsecke !=max_sechsecke)))
  { fprintf(stderr,"split must be called together with code 1 or 5 and without stdout \n");
    fprintf(stderr,"Only graphs of one size may be generated. \n");
    exit(117); }
...
if (!quiet)
  {
    ...
    if (codenumber==3) fprintf(logfile,"Listenlaenge: %d \n",listenlaenge);
    fclose(logfile);
  }
...
if (!quiet) 
  { fprintf(stderr,"Time for case 1 (Bauchbinde): %.1f seconds \n",(double)(buffertime-savetime)/time_factor);
    logfile=fopen(logfilename,"a");
    fprintf(logfile,"Time for case 1 (Bauchbinde): %.1f seconds \n",(double)(buffertime-savetime)/time_factor);
    fclose(logfile);
  }
savetime=buffertime;
}
...
buffertime= TMS.tms_utime;
if (!quiet) 
  { fprintf(stderr,"Time for case 2 (Brille): %.1f seconds \n",(double)(buffertime-savetime)/time_factor);
    logfile=fopen(logfilename,"a");
    fprintf(logfile,"Time for case 2 (Brille): %.1f seconds \n",(double)(buffertime-savetime)/time_factor);
    fclose(logfile);
    savetime=buffertime;
  }
}
...
if (codenumber==3) 
  for (i=20+(2*min_sechsecke); i<=knotenzahl; i++) ausgabe(codeliste+i,i,lastcode[i]);
...
if (!quiet) 
 { 
   ...
   for (i=20+(2*min_sechsecke); i<=20+(2*max_sechsecke); i+=2)
     fprintf(stderr,"Generated %d maps on %d vertices -- reduced to %d non-isomorphic maps. \n",non_iso_graphenzahl[i],i,graphenzahl[i]);
   ...
   for (i=20+(2*min_sechsecke); i<=20+(2*max_sechsecke); i+=2)
     fprintf(logfile,"Generated %d maps on %d vertices -- reduced to %d non-isomorphic maps. \n",non_iso_graphenzahl[i],i,graphenzahl[i]);
   if (spistat) for (i=0; i<=12*max_sechsecke+120; i++) 
