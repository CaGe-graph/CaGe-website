...
/*  Compileraufruf auf SGIs:
   cc CaGe.c -I/vol/local/include -L/vol/local/lib -lm -ltcl -ltk -lX11
   Compileraufruf auf gemini (Linux):
   cc CaGe.c -I/usr/include -I/usr/include/tcl -L/usr/x11/lib -L/usr/lib
   -lm -ltcl -ltk -lX11
 */
...
/*vs */
"      elseif {$programm==3} {\n"
"          wm deiconify .workmess\n"
"          set totkview($programm) 0\n"
"          set file2d($programm) 2\n"
"      }\\\n"
/*blockiert Schlegel bei HCgen */
...
/*vs urspruenglich 
   "      if {$automatic_2d($programm)} {fuelle_dateifeld\n"
   blockiert Schlegel bei HCgen */
...
/*vs */
char Tcl_Skript_HCgen_work_message[] =
"toplevel .workmess\n"
"wm title .workmess \"Message\"\n"
/*  "wm geometry .workmess 400x150\n" */
"wm geometry . +10+10\n"
"wm withdraw .workmess\n"
"frame .workmess.block1\n"
"pack .workmess.block1\n"
"label .workmess.image -image $at_work\n"
"button .workmess.cancel -text Cancel -command {wm withdraw .workmess}\n"
"label .workmess.message -text \"Schlegel diagrams will be soon available for HCgen.\" \n"
"pack .workmess.image .workmess.message .workmess.cancel -padx 2m -pady 2m -in .workmess.block1\n"
/*blockiert Schlegel bei HCgen */
 ;
...
"      set r3call \"plantri_md6 [expr ($n_end(2)+2)/4] -d\"\n"
"  } else {\n"
"    set i 4\n"
"    foreach f $face_list {\n"
"      if {$f == $i} {\n"
...
"       set r3call \"plantri -m4 [expr ($n_end(2)+2)/4] -d\"\n"
"    } else {\n"
...
void schluss (int exitcode)
{
  ...
  if (exitcode != 0) {		/* Fehler */
    ...
    Tcl_VarEval (interp, (char *) "exit 0", (char *) NULL);
  }
...
/***************SCHREIBE_GEWAEHLTEN_ZYKLUS******************************/
/* Diese Funktion liest den vom Benutzer im Schlegel-Diagramm angewaehlten
   Randkreis aus und schreibt ihn nach f. "gcoords" zeigt auf ein Array mit
   2-dimensionalen Koordinaten. (x,y) ist der vom Benutzer angeklickte
   Punkt im Schlegel-Diagramm. "g" ist der Graph im Planarcode.        */

BOOL schreibe_gewaehlten_zyklus (FILE * f, unsigned short *g, double *gcoords)
{
  ...
  if (set) {			/* sonst Aussenflaeche angeklickt => nichts passiert */
    ...
    imin = i;
    jmin = j;
    /* nun ist (imin,jmin) die erste gerichtete Kante */
    /* der Algorithmus ist derselbe wie bei "schreibe_zyklus2" */
    /* es geht immer rechtsherum */
    sprintf (c, "%d %d ", i, j);
    k = 2;			/* Groesse des Kreises */
    while (j != imin) {		/* j ist der aktuelle Knoten, i der Vorgaenger */
      ii = 0;
      i2 = -1;
...
void schreibe_zyklus2(FILE *f,unsigned short *g,int size) {
  ...
  while (knoten<=g[index-1]) {
    first[knoten] = i;  entries[knoten] = 0;
    while (g[i]) {/*fprintf(stderr,"%d ",g[i]);*/ entries[knoten]++; i++;}
    i++;  knoten++;  /*fprintf(stderr,"\n");*/
  } 
  ...
  while (k<size && knoten<=g[index-1]) {
    ...
    i = knoten;   j = g[first[i]+ki];
    sprintf(c,"%d %d ",i,j); 
    k = 2;    /* Groesse des Kreises */
    while (j!=knoten) {   /* j ist der aktuelle Knoten, i der Vorgaenger */
      ii = 0;  i2 = -1; 
      while (ii<entries[j] && i2==-1) { 
        if (g[first[j]+ii]==i) {i2=(ii+entries[j]-1)%entries[j];}
        ii++;
      }
      if (i2==-1) {schluss(88);}
      i = j;  j = g[first[j]+i2]; 
      if (j!=knoten) {k++; sprintf(d,"%d ",j); strcat(c,d);}
    }
    ki++;  if (ki>=entries[knoten]) {ki=0; knoten++;}
  }
  fprintf(f,"%d %s\n",k,c);
  if (ferror(f)) {schluss(75);}
}
...
/* end vs */
	...
	if (!(graphstate[spr_ebindex] & SPRING_EINGEBETTET) ||
	  ...
	  switch (programm) {
	    case FULLGEN:{
		spring = popen ((char *) "spring -ip -o < CaGe1.tmp", "r");
		break;
	      }
	    case DREGGEN:{
		spring = popen ((char *) "spring -ip -o < CaGe1.tmp",
				"r");
		break;
	      }
	    case HCGEN:{
		if (False && p_3 == 0) {	/* eigene Einbettung (z.Zt. gesperrt) */
		  bette_sechseckmuster_ein (graph[spr_ebindex],
				     rasmol_coords[spr_ebindex], woh_3 == 1);
		  if ((f = fopen ((char *) "CaGe1.tmp", "w")) == nil) {
		    schluss (145);
		  }
		  if (write_writegraph2d (f, rasmol_coords[spr_ebindex],
					  graphnv[spr_ebindex], 3, FALSE,
				      TRUE, TRUE, graph[spr_ebindex]) != 1) {
		    schluss (146);
		  }
		  fclose (f);
		  spring = popen ((char *)
			   "spring -n0,0,50 -t -o < CaGe1.tmp", "r");
		} else {
		  spring = popen ((char *)
			   "spring -n30,400,20 -ip -o < CaGe1.tmp", "r");
		}
		break;
	      }
	    case TUBETYPE:{
		spring = popen ((char *) "spring -n30,400 -ip -o < CaGe1.tmp",
				"r");
		break;
	      }
	    case INPUTFILE:{
		sprintf (c, "spring %s -o -n%d,%d,%d,%d < CaGe1.tmp",
		  old_embed && (graphstate[spr_ebindex] & SPRING_EINGEBETTET)
		 ? (char *) "" : (char *) "-ip", emb_a, emb_b, emb_c, emb_d);
		spring = popen (c, "r");
		break;
	      }
	  }
	...
	if (!(graphstate[i] & SCHLEGEL_EINGEBETTET) ||
	  ...
	  fclose (f);
	  schlegel = popen ((char *) "schlegel -o < CaGe2.tmp", "r");
	...
	set = schreibe_gewaehlten_zyklus (f, graph[sl_ebindex],
...
void fullgen_handling (int programm)
{
  ...
  char Tcl_Skript_LoescheAuswahlfenster3[] = "wm withdraw .hctop; wm withdraw .workmess\n";
  char Tcl_Skript_EntferneAuswahlfenster3[] = "destroy .hctop; destroy .workmess\n";
  ...
  switch (programm) {
	...
	if (Tcl_Eval (interp, Tcl_Skript_HCgen_work_message) == TCL_ERROR) {
	  schluss (3011);
	}
	break;
      }
